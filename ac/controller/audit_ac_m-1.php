...
function fn_synthese_audit_orbit_m_1()
{
...
and datediff(month,rm.create_dt,getdate())=1) 'NbRIM',
(select count(acct_no) 
from la_acct la
where 
    rsm.employee_id=empl_id
and datediff(month,la.application_dt,getdate())=1) 'NbDisb',
(select 
...
and datediff(month,gb.create_dt,getdate())=1) 'NbClot',
(select 
...
and rsm.employee_id=gb.empl_id
and datediff(month,gb.create_dt,getdate())=1) 'NbRev',
(select count(acct_no) 
from ln_acct ln 
where 
 rsm.employee_id=empl_id
 and datediff(month,(select create_dt from ln_history where tran_code=350 and reversal=0 and acct_no=ln.acct_no),getdate())=1
...
function fn_synthese_audit_crm_m_1()
{
				include('../../../cnx/database.php');
...
AND PERIOD_DIFF(DATE_FORMAT( CAST(NOW() AS DATE) , '%Y%m' ),DATE_FORMAT(CAST(v1.`createdtime` AS DATE) , '%Y%m' ))=1
AND deleted=0
AND v5.contactid=v1.crmid
AND CONCAT(v3.`last_name`,' ', v3.`first_name`) LIKE '%".$nom."%'
...
function fn_synthese_audit_m_1()
{
	$nom=str_replace('%',' ',$_GET['nom']);
	$iduser=$_GET['iduser'];
	$orbit = file_get_contents('http://10.80.1.51/webreporting/ac/controller/api/fn_synthese_audit_orbit_m_1?iduser='.$iduser);
	$crm = file_get_contents('http://10.80.1.51/webreporting/ac/controller/api/fn_synthese_audit_crm_m_1?nom='.$nom);
