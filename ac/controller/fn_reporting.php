<?php ini_set("display_errors",0); 
		...
		function pen_dmd_130()
				{				
							...
							$sql = "select count(la.ACCT_NO) 'dmd'
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm, ad_gb_branch br
									where rm.rim_no=la.RIM_NO and rsm.BRANCH_NO=br.branch_no 
									and rsm.employee_id=la.APPLICATION_RSM_ID and rsm.branch_no=130
		...
		function pen_dmd_120()
				{				
							...
							$sql = "select count(la.ACCT_NO) 'dmd'
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm, ad_gb_branch br
									where rm.rim_no=la.RIM_NO and rsm.BRANCH_NO=br.branch_no 
									and rsm.employee_id=la.APPLICATION_RSM_ID and rsm.branch_no=120
									and datediff(day,la.application_dt,getdate())<90
									and la.status  in ('Pending') ";
					$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
					$return = array();
		...
		function pen_dmd_110()
				{				
							...
							$sql = "select count(la.ACCT_NO) 'dmd'
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm, ad_gb_branch br
									where rm.rim_no=la.RIM_NO and rsm.BRANCH_NO=br.branch_no 
									and rsm.employee_id=la.APPLICATION_RSM_ID and rsm.branch_no=110
									and datediff(day,la.application_dt,getdate())<90
									and la.status  in ('Pending') ";
					$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
					$return = array();
		...
		function vl_cr_month_130()
				...
				/*08/02/2016 Ajout de delai de traitement demande*/
		function delai_110()
				{				
							include('../../../cnx/database.php');
							$pdo = Database::connect();
							...
							$sql = "select 
									...
									AVG(datediff(day,la.application_dt,(select lh.create_dt from ln_history lh
									  where lh.acct_no = ln.acct_no and lh.tran_code = 350 
									  and lh.reversal = 0
									  ))) 'dmd'
									  
									  from ad_gb_rsm rsm , la_acct la,ln_acct ln,rm_acct rm
									  where rsm.branch_no=110 
									  and  rm.rim_no=ln.rim_no and rm.rsm_id=rsm.employee_id and la.ln_acct_no=ln.acct_no 
									  and datediff(month,(select lh.create_dt from ln_history lh
									  where lh.acct_no = ln.acct_no and lh.tran_code = 350 
									  and lh.reversal = 0
									  ),getdate())<3";
