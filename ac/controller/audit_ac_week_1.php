...
function fn_synthese_audit_orbit_week_1()
{
...
and datediff(week,rm.create_dt,getdate())=1) 'NbRIM',
(select count(acct_no) 
from la_acct la
where 
    rsm.employee_id=empl_id
and datediff(week,la.application_dt,getdate())=1) 'NbDisb',
(select 
...
and datediff(week,gb.create_dt,getdate())=1) 'NbClot',
(select 
...
and rsm.employee_id=gb.empl_id
and datediff(week,gb.create_dt,getdate())=1) 'NbRev',
(select count(acct_no) 
from ln_acct ln 
where 
 rsm.employee_id=empl_id
 and datediff(week,(select create_dt from ln_history where tran_code=350 and reversal=0 and acct_no=ln.acct_no),getdate())=1
...
function fn_synthese_audit_crm_week_1()
{
				include('../../../cnx/database.php');
...
function fn_synthese_audit_week_1()
{
	$nom=str_replace('%',' ',$_GET['nom']);
	$iduser=$_GET['iduser'];
	$orbit = file_get_contents('http://10.80.1.51/webreporting/ac/controller/api/fn_synthese_audit_orbit_week_1?iduser='.$iduser);
	$crm = file_get_contents('http://10.80.1.51/webreporting/ac/controller/api/fn_synthese_audit_crm_week_1?nom='.$nom);
