...
,(select count(ln.acct_no) from la_acct ln , rm_acct rm where rm.rim_no=ln.rim_no and rm.rsm_id=rsm.employee_id and ln.status in ('approved') and datediff(day,ln.APPLICATION_DT,getdate())<90) as approved
...
and rsm.employee_id=rm.rsm_id 
and dis.status in ('Active','NonAccrual') 
and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) >10) as per10

,(select  isnull (sum(dis.cur_bal),0) from rm_acct rm, ln_display dis,ov_control ov
where rm.rim_no =  dis.rim_no
and rsm.employee_id=rm.rsm_id 
and dis.status in ('Active') 
and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) between 11 and 90) as per10prime
...
foreach ($result as $row){
	...
	if ($row['volclt']==0) $row['per10']='-'; else $row['per10']=number_format(($row['per10']/$row['volclt'])*100,2,'.','').'%';
	if ($row['volclt']==0) $row['per10prime']='-'; else $row['per10prime']=number_format(($row['per10prime']/$row['volclt'])*100,2,'.','').'%';
