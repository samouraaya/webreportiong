...
(select sum(amt) from ln_history where acct_no=ln.acct_no and tran_code in (300) and datediff(month,create_dt,ov.last_to_dt)=0) 'RECMOIS',
...
and lh.reversal = 0 
and datediff(month,lh.create_dt,ov.last_to_dt)=0
...
foreach ($result as $row){
    ...
    $return[]=array('RSM'=>$row['RSM'],
					...
					'SOLDECT'=>$row['SOLDECT'],
					'RECMOIS'=>$row['RECMOIS'],
					'RESTAPAYEPEN'=>$row['RESTAPAYEPEN'],
					'RESTAPAYEINT'=>$row['RESTAPAYEINT'],
					'RESTAPAYECAP'=>$row['RESTAPAYECAP']
					);
}
