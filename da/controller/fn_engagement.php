...
from engage where status=0";

$result = $pdo1->query($sql)->fetchAll(PDO::FETCH_ASSOC);
$return = array();
foreach ($result as $row){
	$rim=$row['Rim'];
	$sql1="select name 'rsm' from ad_gb_rsm rsm ,rm_acct rm where rm.rsm_id=rsm.employee_id and rm.rim_no=$rim and rsm.branch_no=".$agence."";
