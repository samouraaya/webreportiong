...
(SELECT COUNT(crmid) 
		...
		AND v4.converted=0
		AND SUBSTR(cf_987,1,3)=".$agence."
		AND v1.`crmid`=v4.`leadid`))) 'nbprospect'
...
{
	$agence=$_GET['agence'];
	$orbit = file_get_contents('http://10.80.1.51/webreporting/da/controller/api/fn_synthese_audit_orbit?agence='.$agence);
	$crm = file_get_contents('http://10.80.1.51/webreporting/da/controller/api/fn_synthese_audit_crm?agence='.$agence);
...
function fn_perfermance_ac_crm()
{
				include('../../../cnx/database.php');
				$pdo = Database3::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

				$sql = "SELECT CONCAT(v3.`last_name`,' ', v3.`first_name`) 'name',id_orbit 'orbit',
						(SELECT COUNT(crmid)
...
function fn_stock_relance_ac_agence()
{
	$agence=$_GET['agence'];
	$orbit = file_get_contents('http://10.80.1.51/webreporting/da/controller/api/fn_liste_ac_orbit?agence='.$agence);
	$crm = file_get_contents('http://10.80.1.51/webreporting/da/controller/api/fn_perfermance_ac_crm');
	...
	foreach($json_b  as $crm)
	{
		if ($orbit['employee_id']==$crm['orbit'])
		{
			$return[]=array(
					'name'=>$crm['name'],
					'TotalActif'=>$crm['TotalActif'],
					'NbRelance'=>$crm['NbRelance'],
					'RELANCEWEEK'=>$crm['RELANCEWEEK'],
					'RELANCEWEEK1'=>$crm['RELANCEWEEK1'],
					'RELANCEMONTH1'=>$crm['RELANCEMONTH1'],);
		break;
		}
