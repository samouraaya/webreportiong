...
/*
	c'est une fonction qui permet de retourner la liste des échéances futures
*/
function liste_ech_fut()
{
...
,cp.name 'rsm'
,(select substring(description,8,LEN(description)) from ad_ln_cls where class_code=cr.class_code) 'Produit'
,cast(dp.nxt_pmt_dt as date)  'date_échéance'
...
and cl.class_code <>190 and cl.branch_no =".$agence." 
having (select min(bill_id_no) from ln_bill where acct_no=cr.acct_no and status='Unsatisfied ')=bl1.bill_id_no order by cp.name,'date_échéance'";
