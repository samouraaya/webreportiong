...
where  dis.status not in ('closed','Incomplete','Active charge-off')
	...
	*/
	...
	{				
							...
							$sql="select  
								...
								where dis.status not in ('closed','Incomplete','Active charge-off')
								and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) between 11 and 180
								   ";
				$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
				$return = array();
				foreach ($result as $row1)
	...
	*/
	...
	{				
							...
							from  ln_display dis,ov_control ov
							where dis.status not in ('closed','Incomplete','Active charge-off')
							and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) between 31 and 180
...
where  datediff(day,la.application_dt,getdate())<90
and rm.rim_no=la.rim_no
and rm.branch_no=br.branch_no
and la.status in ('pending')) 'nbpending',
(select  count(la.acct_no) 
from la_acct la, rm_acct rm
where datediff(day,la.application_dt,getdate())<90
and datediff(day,la.application_dt,getdate())>10
and rm.rim_no=la.rim_no
and rm.branch_no=br.branch_no
...
where  datediff(month,(select lh.create_dt from ln_history lh
where lh.acct_no = ln.acct_no and lh.tran_code = 350 and lh.reversal = 0) ,getdate())=0 
...
and ln.status in ('Active')
and rm.rim_no=ln.rim_no
and rm.branch_no=br.branch_no) 'vldec',
(select count(acct_no)   from ln_acct ln,rm_acct rm where ln.status not in ('closed','Incomplete','Active charge-off')and rm.rim_no=ln.rim_no
and rm.branch_no=br.branch_no) 'nbclt',
(select sum(dis.cur_bal) from ln_display dis,rm_acct rm where  dis.status not in ('closed','Incomplete','Active charge-off')and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'vlclt',
(select  
count(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 0 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'NBPAR0',
(select  
count(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 10 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'NBPAR10',
(select  
count(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 30 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'NBPAR30',
(select  
sum(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 0 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'Vlpar0',
(select  
sum(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 10 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'vlpar10',
(select  
sum(dis.cur_bal)
from ln_display dis,ov_control ov,rm_acct rm
where dis.status not in ('closed','Incomplete','Active charge-off') and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt)) > 30 and rm.rim_no=dis.rim_no
and rm.branch_no=br.branch_no) 'VLpar30'

from ad_gb_branch br
where branch_no=".$agence."
							";
				$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
				$return = array();
				foreach ($result as $row1)
