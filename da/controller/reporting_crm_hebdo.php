...
$sql1 = "select name,employee_id 'id_cc' from ad_gb_rsm where status='active' and empl_class_code=310 and branch_no=".$agence."";
...
$pdo1 = Database::connect();
				$pdo1->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
				$agence=$_GET['agence'];  
$sql1 = "select name,employee_id 'id_cc' from ad_gb_rsm where status='active' and empl_class_code=310 and branch_no=".$agence." order by name";
$result1 = $pdo1->query($sql1)->fetchAll(PDO::FETCH_ASSOC);
$return = array();
foreach ($result1 as $row1){				
$sql = "SELECT CONCAT(v3.`last_name`,' ', v3.`first_name`) 'name'
...
$sql = "select name,employee_id 'id_cc' from ad_gb_rsm where status='active' and empl_class_code=310 and branch_no=".$agence."";
$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
$return = array();
