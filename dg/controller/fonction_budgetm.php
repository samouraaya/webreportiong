...
,(select count(lb.acct_no) 
                  ...
                  and datediff(month,lb.pmt_due_dt,ov.last_to_dt)=0
...
,(select count(la.acct_no) from la_acct la where datediff(month,la.application_dt,ov.last_to_dt)=0 and la.branch_no=br.branch_no) 'nb_loan_app'
,(select count(la.acct_no) from la_acct la where datediff(month,la.close_dt,ov.last_to_dt)=0 and datediff(month,la.application_dt,ov.last_to_dt)=0 and la.branch_no=br.branch_no) 'nb_loan_closed'
...
and datediff(month,(select lh.create_dt from ln_history lh where lh.acct_no = ln.acct_no and lh.tran_code = 350 and lh.reversal = 0),ov.last_to_dt)=0 ) 'ln_refinanced'
,(select count(ln.acct_no) from ln_acct ln where ln.status in ('closed') and ln.branch_no=br.branch_no and datediff(month,(select lh.create_dt from ln_history lh where lh.acct_no = ln.acct_no and lh.tran_code = 345 and lh.reversal = 0),ov.last_to_dt)=0) 'ln_closed'
...
,(select count(lb.acct_no) 
                  ...
                  and datediff(month,lb.pmt_due_dt,ov.last_to_dt)=0
									and ln.status in ('Active','NonAccrual')) 'EchWillBe'
,(select count(la.acct_no) from la_acct la where datediff(month,la.application_dt,ov.last_to_dt)=0 ) 'nb_loan_app'
,(select count(la.acct_no) from la_acct la where datediff(month,la.close_dt,ov.last_to_dt)=0 and datediff(month,la.application_dt,ov.last_to_dt)=0 ) 'nb_loan_closed'
,(select count(ln.acct_no) from ln_acct ln where ln.status not in ('incomplete','closed') 
and ln.refinanced='N' and (select count(acct_no) from ln_acct where rim_no=ln.rim_no and status='Closed')>0 
and datediff(month,(select lh.create_dt from ln_history lh where lh.acct_no = ln.acct_no and lh.tran_code = 350 and lh.reversal = 0),ov.last_to_dt)=0 ) 'ln_refinanced'
,(select count(ln.acct_no) from ln_acct ln where ln.status in ('closed') and datediff(month,(select lh.create_dt from ln_history lh where lh.acct_no = ln.acct_no and lh.tran_code = 345 and lh.reversal = 0),ov.last_to_dt)=0) 'ln_closed'
from ov_control ov
";
				$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
				$return = array();
