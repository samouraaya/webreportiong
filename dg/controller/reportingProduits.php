...
$sql = "select br.branch_no 'agence'
,isnull((select sum(cur_bal) 
from ln_display a,rm_acct b ,ln_acct c,ad_gb_rsm  d,ad_ln_cls cls
where a.acct_no = c.acct_no
and c.class_code = cls.class_code
and cls.class_code =".$id_produit."
and b.rim_no = c.rim_no
and c.status not in ('Closed','incomplete','active charge-off')
and b.branch_no = br.branch_no   
...
,(select count(lb.acct_no) 
									from ln_bill lb, ln_acct ln, rm_acct rm, ad_gb_rsm rsm,ad_ln_cls cls
                  ...
                  and cls.class_code=".$id_produit."
							...
							from ad_gb_rsm cp, ln_acct cr, rm_address ad, rm_acct cl, ln_bill bl1,ln_pmt_schedule sh,ln_display dp,dp_display lo,ad_ln_cls cls
							where cp.employee_id = cl.rsm_id
							and cl.rim_no = cr.rim_no
              and cr.class_code=cls.class_code
              and cls.class_code=".$id_produit."
							and cr.rim_no = ad.rim_no
							and dp.acct_no = bl1.acct_no
              and cl.branch_no = br.branch_no
...
,isnull((select  
									count(cur_bal) 
									from rm_acct rm, ln_display dis, ov_control ov,ad_ln_cls cls
...
,isnull((select  
                  ...
                  and cls.class_code=".$id_produit."
									and dis.status not in ('Closed','Incomplete','active charge-off')
									and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt))>10 ),0) 'nbpar7'
,isnull((select  
									sum(cur_bal) 
									from rm_acct rm, ln_display dis, ov_control ov,ad_ln_cls cls
									where rm.rim_no = dis.rim_no
                  ...
                  and cls.class_code=".$id_produit."
									and dis.status not in ('Closed','Incomplete','active charge-off')
									and datediff(dd,dis.delq_dt,dateadd(dd,1,ov.last_to_dt))>10 ),0) 'vlpar7'
,isnull((select  
									count(cur_bal) 
									from rm_acct rm, ln_display dis, ov_control ov,ad_ln_cls cls
									where rm.rim_no = dis.rim_no
...
Database::disconnect();
	...
	function demande_reporting_produit ()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
,isnull((select count(la.ACCT_NO) 
				  ...
				  and la.class_code =".$id_produit."
									and datediff(day,la.application_dt,getdate())<90),0)+isnull((select count(la.ACCT_NO) 
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm,ln_acct ln
									where rm.rim_no=la.RIM_NO 
                  and rm.branch_no=br.branch_no 
				  ...
				  and la.class_code =".$id_produit."
									and datediff(day,ln.create_dt,getdate())<30),0) 'nbstock'
,isnull((select count(la.ACCT_NO) 
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm
									where rm.rim_no=la.RIM_NO 
                  and rm.branch_no=br.branch_no 
...
,isnull((select count(la.ACCT_NO) 
				  ...
				  and ln.status='Incomplete'
				  and la.class_code =".$id_produit."
									and datediff(day,ln.create_dt,getdate())<30),0) 'nbstockapp'
,isnull((select sum(la.application_amt) 
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm
									where rm.rim_no=la.RIM_NO 
                  and rm.branch_no=br.branch_no 
				  and la.class_code =".$id_produit."
									and rsm.employee_id=la.APPLICATION_RSM_ID
                  and la.STATUS not in('closed','disbursed')
									and datediff(day,la.application_dt,getdate())<90),0)+isnull((select sum(la.approval_amt) 
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm,ln_acct ln
									where rm.rim_no=la.RIM_NO 
                  and rm.branch_no=br.branch_no 
				  and ln.status='Incomplete'
                  ...
                  and la.STATUS='disbursed'
				  and la.class_code=".$id_produit."
									and datediff(day,ln.create_dt,getdate())<30),0) 'vlstock'
,isnull((select sum(la.application_amt)
									from LA_ACCT la, rm_acct rm,ad_gb_rsm rsm
									where rm.rim_no=la.RIM_NO 
                  and rm.branch_no=br.branch_no 
...
,isnull((select sum(la.approval_amt)
                  ...
                  and la.STATUS='disbursed'
				  and la.class_code=".$id_produit."
									and datediff(day,ln.create_dt,getdate())<30),0) 'vlstockapp'
...
foreach ($result as $row)
	{
				$return[]=array(
					...
					'vlstock'=>number_format($row['vlstockpen']+$row['vlstockrev']+$row['vlstockapp'],'0','.',' '),
...
Database::disconnect();
	...
	function demande_reporting_total_produit()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
,isnull((SELECT  
                  ...
                  and rsm.employee_id=rm.rsm_id 
									...
									and cast(la.close_DT as date) = cast(getdate() as date)),0) 'nbdmdclo'
,isnull((select count(la.ACCT_NO) 
									from LA_ACCT la, rm_acct rm,ad_ln_cls cls
									where rm.rim_no=la.RIM_NO 
                  and la.class_code =cls.class_code
                  and cls.class_code=".$id_produit."
                   
								
                  and la.STATUS not in('closed','disbursed')
									and datediff(day,la.application_dt,getdate())<90),0)+isnull((select count(la.approval_amt) 
									from LA_ACCT la, rm_acct rm,ln_acct ln,ad_ln_cls cls
									where rm.rim_no=la.RIM_NO 
                  and cls.class_code=la.CLASS_CODE
                  and cls.class_code=".$id_produit."
...
Database::disconnect();
	...
	function cumul_week_reporting_produit()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
Database::disconnect();
	...
	function cumul_week_reporting_total_produit()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
Database::disconnect();
	...
	function cumul_month_reporting_produit()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
Database::disconnect();
	...
	function cumul_month_reporting_total_produit ()			
	{
		include('../../../cnx/database.php');
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
