...
function fn_avg_trait_110()
{
				...
				$sql = "select AVG (datediff(day,la.application_dt,(select lh.create_dt from ln_history lh
						  ...
						  and lh.reversal = 0
						  ))) 'nb' from la_acct la,ln_acct ln,rm_acct rm where rm.rim_no=ln.rim_no and rm.branch_no=110 and la.ln_acct_no=ln.acct_no";
				$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
				$return = array();
				foreach ($result as $row1)
				$return[]=array('nb'=>number_format($row1['nb'],0,'.',' '));
