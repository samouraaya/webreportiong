...
and la_acct.Purpose_id*=ad_ln_purpose.Purpose_id and la_acct.status not in ('Disbursed','Closed')   order by ad_gb_rsm.branch_no,ad_gb_rsm.name";

$result = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
$return = array();
foreach ($result as $row){
	$row['nomclient']=iconv("UTF-8","ISO-8859-2//IGNORE",$row['nomclient']);
    $return[]=array('rim'=>$row['RIMNO'],
					'age'=>$row['age'],
					'numdem'=>$row['numdem'],
					'rsm'=>$row['nomRSM'],
