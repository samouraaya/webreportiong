...
,(select cast(max(nxt_bill_dt) as date) from ln_pmt_schedule where acct_no=a.acct_no and status in ('Active'))   'dateécheance'
...
(select create_dt from ln_history where tran_code=350 and reversal=0 and acct_no=a.acct_no) 'date_dec',
convert(varchar(8),(select min(distinct bill_id_no) from ln_bill where acct_no=a.acct_no and status='Unsatisfied' and pmt_due_dt<getdate()))+'/'+convert(varchar(8),a.trm) 'nbech'
...
WHERE  a.status in  ('Active', 'NonAccrual' )
			   ...
			   and f.user_name like '%" . $user . "%'
			   and sch.ptid=(select max(ptid) from ln_pmt_schedule where acct_no=sch.acct_no and status='Active'  and first_pmt_dt<getdate())
